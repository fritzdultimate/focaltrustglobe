    ...
    <main class="app-content">
        ...
        <div class="page-title m-b-20">
            ...
            <div class="row justify-content-between">
                <div class="col-md-2">
                    <h1 class="page-title">
                        <span class="fe fe-users"></span> Withdrawals
                    </h1>
                </div>
            </div>
        </div>
                        ...
                        <div class="row">
                            <div class="col-md-8">
                                <div class="btn-group w-30 m-b-10">
                                    <a href="/admin/withdrawals" class="btn active">
                                        All 
                                        <span class="badge badge-pill bg-azure">{{ number_format($withdrawal_count) }}</span>
                                    </a>
                                    <a href="/admin/withdrawals?filter=pending" class="btn ">
                                        Pending 
                                        <span class="badge badge-pill bg-indigo">{{ number_format($pending_count) }}</span>
                                    </a>
                                    <a href="/admin/withdrawals?filter=accepted" class="btn ">
                                        Accepted 
                                        <span class="badge badge-pill bg-orange">{{ number_format($accepted_count) }}</span>
                                    </a>
                                    <a href="/admin/withdrawals?filter=denied" class="btn ">
                                        Denied 
                        ...
                        <table class="table table-hover table-bordered table-vcenter card-table">
                            ...
                            <tbody class="">
                                ...
                                <tr class="tr_1b667862cb93ecba7c392f57bb29763b">
                                    ...
                                    <td>
                                        <div class="title"><h6>{{ ucfirst($withdrawal->user->firstname) }} {{ $withdrawal->user->lastname }} ({{ $withdrawal->user->name }})</h6></div>
                                        <div class="sub text-muted">{{ $withdrawal->user->email }}</div>
                                    </td>
                                    <td class="text-center w-10p">{{ number_format($withdrawal->amount, 2) }}</td>
                                    <td class="text-center w-10p">{{ $withdrawal->status }}</td>
                                    <td class="text-center w-10p">{{ ucfirst($withdrawal->user_wallet->admin_wallet->currency) }}</td>
                                    <td class="text-center w-10p">{{ $withdrawal->transaction_hash }}</td>
                                    <td class="text-center w-15p">{{ get_day_format($withdrawal->created_at) }}</td>
                                    <td class="text-center w-15p">{{ get_day_format($withdrawal->updated_at) }}</td>
                                    <td class="text-center w-5p">
                                        <div class="item-action dropdown">
                                            ...
                                            <div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(9px, 16px, 0px); top: 0px; left: 0px; will-change: transform;" x-out-of-boundaries="">
                                                <a href="#" class="dropdown-item approve-withdrawal-btn" data-id="{{ $withdrawal->id }}">
                                                    <i class="dropdown-icon fe fe-checkmark-2"></i> 
                                                    Approve
                                                </a>
                                                <a href="#" class="dropdown-item deny-withdrawal-btn" data-id="{{ $withdrawal->id }}">
                                                    <i class="dropdown-icon fe fe-close-2"></i> 
                                                    Deny
                                                </a>
                                                <a href="#" class="dropdown-item delete-withdrawal-btn" data-id="{{ $withdrawal->id }}">
                                                    <i class="dropdown-icon fe fe-trash-2"></i> 
                                                    Delete
                                                </a>
                                            </div>
...
<script>
    ...
    [...document.querySelectorAll('.approve-withdrawal-btn')].forEach(el => {
        el.addEventListener('click', (e) => {
            console.log(e)

            if (confirm("This withdrawal will be approved, continue?") == true) {
                fetch(urlPrefix + `admin/withdrawal/approve`, {
                    method : 'post',
                    headers,
                    body : JSON.stringify({
    ...
    [...document.querySelectorAll('.deny-withdrawal-btn')].forEach(el => {
        el.addEventListener('click', (e) => {
            console.log(e)

            if (confirm("This withdrawal will be denied, continue?") == true) {
                fetch(urlPrefix + `admin/withdrawal/deny`, {
                    method : 'post',
                    headers,
                    body : JSON.stringify({
                    id : e.currentTarget.dataset.id
    ...
    [...document.querySelectorAll('.delete-withdrawal-btn')].forEach(el => {
        el.addEventListener('click', (e) => {
            console.log(e)

            if (confirm("This withdrawal will be deleted permanently, continue?") == true) {
                fetch(urlPrefix + `admin/withdrawal/delete`, {
                    method : 'post',
                    headers,
                    body : JSON.stringify({
                    id : e.currentTarget.dataset.id
